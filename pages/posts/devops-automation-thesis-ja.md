...
現代のソフトウェア開発において、リリースの迅速化とソースコードの技術的な品質担保は、しばしば相反する二つの力として捉えられます。ビジネス側がこれまで以上に早い機能リリースを求める一方で、運用の安定性を保つためには厳格なプロセスと幾重もの安全策が必要となるからです。

このジレンマに対処するため、私は **ENI École** での **Manageur de Solutions Digitales et Data (MS2D)** 修士学位取得に向けた研究の一環として、本テーマに取り組みました。私の会社 (あるITサービス企業) のレンヌ支店にて、企業でのメンターに支えられながら、私は以下に示す核心的な問いを中心に、包括的な自動化戦略を計画・実施しました。

> 「開発チームの生産性を向上させるために、ソフトウェア開発ライフサイクル（SDLC）の各フェーズを自動化できるソフトウェアソリューションにはどのようなものがあるか？」

以下は、技術的、組織的、そして人間的な側面に満ちたこの取り組みの振り返り（レトロスペクティブ）です。

---

## 1. 現状分析：摩擦の診断（監査）

やみくもにスクリプトを書き始める前に、まず既存のプロセスを分析する必要がありました。この現状診断を客観的に行うため、私たちは以下の2本の柱からなる監査手法を採用しました。
...
*   **手動による不安の募るデプロイ**：ステージングや本番へのリリースは、何ページにも及ぶ紙の手順書（ランブック）に頼っていました。FTPやSCPでパッケージを転送し、手動でサービスを停止し、SQLスクリプトを手動で実行し、すべてを再起動する――これらを多くの場合、深夜に行っていました。手順を一つでも間違えれば、リリースは即座に失敗しました。
*   **遅すぎるフィードバックループ**：テストの自動実行の仕組みがなかったため、デグレード（先祖返り）やコード品質の不具合はQAフェーズになって初めて発覚するか、最悪の場合は本番環境のユーザーによって発見されていました。バグが発生してから数週間後に修正するのは、非常にコストと時間がかかるものでした。
...
これらの課題を解決するため、私は会社の制約や開発チームのスキルセットに最も適したツールチェーンを選定する比較調査を行いました。選定にあたっては、以下の主要な評価基準からなる厳格なマトリックスを定義しました。
...
### CI/CD：GitLab CIによるネイティブ統合の選択
業界のベテランであるJenkinsや、非常に人気のあるGitHub Actionsなども検討しましたが、最終的には **GitLab CI** を最優先に選びました。当社はすでにオンプレミスのGitLabインスタンスでソースコードを管理していたため、GitLab CIの採用は極めて自然な流れでした。
...
**Kubernetes（K8s）** はコンテナオーケストレーションのデファクトスタンダードとなっていますが、社内プロジェクトを扱う中規模チームにとっては、インフラコストや運用の複雑さが大きすぎました。

そこで、私は以下の理由から **Docker Swarm** の採用を決定しました。
...
### 品質とオブザーバビリティ：SonarQubeとPrometheus / Grafanaのデュオ
ソースコードの品質向上とセキュリティ強化のため、技術的負債や脆弱性、テストカバレッジに関するフィードバックを迅速に得られるよう **SonarQube** を統合しました。

本番環境側におけるオブザーバビリティ（可観測性）は、**Prometheus**（専用のエクスポーターを介してアプリケーションとシステムメトリクスを収集）と **Grafana**（リアルタイムの可視化とSlack/Teamsへのアラート通知用）を中心に構築しました。また、ログの集約には **ELKスタック**（Elasticsearch, Logstash, Kibana）を採用しました。

---

## 3. パイロットプロジェクト：実戦で検証する「Event (社内イベント管理アプリケーション)」

このアーキテクチャを検証するため、私たちは Vue.jsのフロントエンド、Spring Boot (Java)のバックエンド、およびPostgreSQL/MySQLデータベースで構成される代表的な社内アプリケーション **Event (社内イベント管理アプリケーション)** を用いて実証実験を行いました。具体的な取り組みとしては、「ユーザーアカウント管理」モジュールの完全な移行に焦点を当てました。

> [!NOTE]
> この2週間のパイロットスプリント期間中、開発チームがDevOpsエンジニアリングとパイプラインの構築に専念できるよう、マネジメント側は一時的な新規機能開発の凍結（フィーチャーフリーズ）を承諾してくれました。

実運用環境に極めて近い状況で遭遇した主要な技術的課題と、その解決策は以下の通りです。

### 課題1：パイプライン実行の遅さ（20分から8分へ短縮）
...
これにより、ビルド時間全体が **8分** にまで短縮され、チームにとって開発のフィードバックループが格段に迅速かつ快適なものになりました。

### 課題2：不安定なUIテスト（Flaky Tests）
Vue.jsフロントエンドでのエンドツーエンド（E2E）テストにおいて、実際のバグがないにもかかわらず、ブラウザの描画遅延が原因でランダムにテストが失敗する事象が発生しました。
*   **解決策**：固定時間の待機（例: `sleep 2000`）を禁止し、明示的な同期処理（CypressやPlaywrightの動的な `waitFor` 命令）に置き換えました。さらに、CIでの誤検知を防ぐため、自動リトライシステム（失敗時に1回のみ再実行する設定）を実装しました。
...
## 4. 導入成果：数字が示す効果（DORAメトリクスと品質）

数スプリントにわたるパイロットモジュールの運用後に収集された結果は、その成果を雄弁に物語っています。プロセスの効率化と成果物の品質向上に対する自動化の影響が、数値として明確に現れました。
...
これらの指標は、**DORA**（DevOps Research and Assessment）が定義する重要なメトリクスに直結しています。私たちは、リリースの速度を高めつつ、同時にアプリケーションの品質と安定性を劇的に向上させることが可能であることを実証しました。

---

## 5. 今後の展望：次なるステップ

この自動化戦略を当社のすべての開発チームへと横展開する計画が、現在9〜12ヶ月のロードマップに沿って進められています。その先を見据えると、さらにエキサイティングな機会が私たちを待ち受けています。
...
今回の修士論文に向けた取り組みを通じて、自動化とは単に「流行りのツールを導入すること」ではなく、企業の経済的価値および人材のパフォーマンスを向上させるための強力な原動力であるということが実証されました。エンジニアを手作業による反復的かつ不安の募る業務から解放することで、クライアントに真の価値を届けるための本質的な業務――設計や開発――に再びフォーカスしてもらうことができるようになります。

DevOpsへの道は継続的な改善の旅であり、そこでは技術選定と同じくらい、コラボレーションの文化と実践を通じた学習が重要な意味を持つのです。
